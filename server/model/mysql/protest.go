...
`, ray)
	...
	for rows.Next() {
		...
		if atLat != nil && atLng != nil {
			var dist float64
			if err = rows.Scan(&p.ID, &p.Title, &p.Protest, &p.Description, &p.Organizer, &p.Public, &p.GatherAt, &p.CreatedAt, &p.UpdatedAt, &dist); err != nil {
				return out, err
			}
		} else {
			if err = rows.Scan(&p.ID, &p.Title, &p.Protest, &p.Description, &p.Organizer, &p.Public, &p.GatherAt, &p.CreatedAt, &p.UpdatedAt); err != nil {
				return out, err
			}
		}
